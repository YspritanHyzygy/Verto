...
시스템 Vision이 기본 인식 방식이며 PP-OCRv6 모델 팩은 별도의 [`Verto-Model-Packs`](https://github.com/YspritanHyzygy/Verto-Model-Packs) 프로젝트에서 빌드하고 배포합니다. 프로덕션 앱에는 모델 선택기가 없습니다. A12–A13 기기는 Vision만 사용하고, A14–A16은 첫 실행 때 균형형과 경량형 순서로, A17 Pro 이상은 최고 정확도와 균형형 순서로 준비합니다. 다운로드는 백그라운드에서 순차적으로 진행되며 정상 모델이 없으면 촬영을 기다리지 않고 즉시 Vision을 사용합니다. 한국어는 항상 Vision을 사용하고 경량 모델은 일본어 가나를 처리하지 않습니다.

입력 언어가 자동이면 먼저 빠른 Vision과 시스템 언어 인식기로 문자 체계를 확인합니다. 신뢰도가 낮은 텍스트, 한글, 알 수 없는 문자 체계, 모델이 완전히 지원하지 않는 이미지는 정확도 우선 Vision으로 인식합니다. 별도로 설치할 수 있는 `OCR Test` scheme에서만 자동, Vision, 세 모델 등급을 직접 지정할 수 있으며, 오른쪽 위 배지가 다음 촬영에 유효한 엔진을 표시합니다.
...
설정 화면은 현재 빌드가 실제로 사용하는 번역 서비스를 표시합니다. 음성 읽기 방식, 텍스트 화면 자동 읽기, 화면 모드도 설정할 수 있습니다. 프로덕션 OCR 모델은 자동으로 관리되며 테스트 조작은 `OCR Test` 빌드에만 표시됩니다. 자체 모델과 LLM 번역은 비활성화된 로드맵 항목입니다. 설정과 최근 언어 쌍은 UserDefaults에 저장됩니다.
