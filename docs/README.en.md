...
System Vision provides the baseline recognizer, while the separate [`Verto-Model-Packs`](https://github.com/YspritanHyzygy/Verto-Model-Packs) project builds and publishes the PP-OCRv6 packs. Production no longer exposes a model picker: A12–A13 devices use Vision only; A14–A16 devices prepare Balanced and then Lightweight at first launch; A17 Pro and newer devices prepare Highest accuracy and then Balanced. Downloads run sequentially in the background. A capture immediately uses Vision whenever no healthy model is ready. Korean always uses Vision, and the Lightweight model does not handle Japanese kana.
