...
Vision proporciona el reconocimiento base del sistema. Al descargar un paquete de PP-OCRv6, la cámara usa los modelos de detección y reconocimiento convertidos a Core ML. El proyecto independiente [`PP-OCR-for-Apple`](https://github.com/YspritanHyzygy/PP-OCR-for-Apple) compila y publica estos paquetes. En Ajustes se puede elegir entre Ligero, Equilibrado y Máxima precisión. El tamaño de descarga y la puntuación de reconocimiento se leen directamente de [`OCRModelTier`](../Verto/Camera/OCRModelPack.swift). El coreano siempre usa el reconocimiento de texto del sistema. El paquete Ligero no incluye kana japonés, así que el japonés también usa el reconocimiento del sistema con ese nivel.
...
Ajustes muestra el servicio de traducción efectivo de la compilación actual. También controla la lectura por voz, la lectura automática de la pantalla de texto, los modelos OCR y la apariencia. El modelo propio y la traducción con LLM son elementos desactivados de la hoja de ruta. Las preferencias y el par de idiomas más reciente se guardan en UserDefaults.
