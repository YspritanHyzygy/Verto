...
システムの Vision が基本の認識方式で、PP-OCRv6 のモデルパックは独立した [`Verto-Model-Packs`](https://github.com/YspritanHyzygy/Verto-Model-Packs) プロジェクトでビルド、公開されています。本番版にはモデル選択を表示しません。A12–A13 は Vision のみ、A14–A16 は初回起動時にバランス、軽量の順、A17 Pro 以降は最高精度、バランスの順に準備します。ダウンロードはバックグラウンドで順番に進み、正常なモデルがなければ撮影を待たせず Vision を使います。韓国語は常に Vision、軽量モデルは日本語のかなを処理しません。

入力言語が自動の場合、まず高速 Vision とシステムの言語認識で文字体系を確認します。信頼度が低い文字、ハングル、未知の文字体系、モデルが完全に対応できない画像は、高精度 Vision で認識します。別インストール可能な `OCR Test` scheme だけが、自動、Vision、3 段階のモデル上書きと、次回撮影で有効なエンジンを示す右上のバッジを表示します。
...
設定画面には現在のビルドが実際に使う翻訳サービスが表示されます。音声の読み上げ方法、テキスト画面の自動読み上げ、外観も設定できます。本番版の OCR モデルは自動管理され、テスト操作は `OCR Test` ビルドだけに表示されます。独自モデルと LLM 翻訳は無効化されたロードマップ項目です。設定と直近の言語ペアは UserDefaults に保存されます。
