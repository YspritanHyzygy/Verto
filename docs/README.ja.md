...
システムの Vision が基本の認識方式で、PP-OCRv6 のモデルパックは独立した [`PP-OCR-for-Apple`](https://github.com/YspritanHyzygy/PP-OCR-for-Apple) プロジェクトでビルド、公開されています。本番版にはモデル選択を表示しません。A12–A13 は Vision のみ、A14–A16 は初回起動時にバランス、軽量の順、A17 Pro 以降は最高精度、バランスの順に準備します。ダウンロードはバックグラウンドで順番に進み、正常なモデルがなければ撮影を待たせず Vision を使います。韓国語は常に Vision、軽量モデルは日本語のかなを処理しません。
